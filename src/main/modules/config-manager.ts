...
export class ConfigManager {
  ...
  /** 导入现有配置（mac: ~/.bashrc_* ；win: profiles/*.ps1） */
  ...
  /** win：PowerShell profile（$env: 变量） */
  private generatePs1Content(config: ModelConfig): string {
    const e = (s: string) => s.replace(/'/g, "''")
    const lines: string[] = []
    lines.push(`# ${config.name} - managed by cc-switch-plus`)
    lines.push(`$env:ANTHROPIC_AUTH_TOKEN = '${e(config.authToken)}'`)
    lines.push(`$env:ANTHROPIC_BASE_URL = '${e(config.baseUrl)}'`)
    if (config.apiTimeout) lines.push(`$env:API_TIMEOUT_MS = '${config.apiTimeout}'`)
    if (config.haikuModel) lines.push(`$env:ANTHROPIC_DEFAULT_HAIKU_MODEL = '${e(config.haikuModel)}'`)
    if (config.sonnetModel) lines.push(`$env:ANTHROPIC_DEFAULT_SONNET_MODEL = '${e(config.sonnetModel)}'`)
    if (config.opusModel) lines.push(`$env:ANTHROPIC_DEFAULT_OPUS_MODEL = '${e(config.opusModel)}'`)
    if (config.customVars) {
      for (const [key, val] of Object.entries(config.customVars)) {
        lines.push(`$env:${key} = '${e(val)}'`)
      }
    }
